  ...
  void mouseListener() {
    if (isInsideSlider() == true) {
      if (mousePressed) {
        ...
        if (isInteger == true) setInitialValue((int)(getValue()+0.5));
...
class TextField {
  ...
  public void update () {
    ...
    fill (255,255,255,50);
    textAlign (LEFT, CENTER);
    textSize (this.size.y / 1.8);
    pushMatrix ();
    translate (this.pos.x, this.pos.y);
    rect (0,0, size.x, size.y);
    fill (250, 240);
    text (content, this.size.x / 2.0, this.size.y / 2.0);
    textAlign (LEFT, CENTER);
    text (name, this.size.x + 5, this.size.y / 2.0);
    popMatrix ();
  }
...
class Menu {
  ...
  public Menu () {
    ...
    fps = new TextField ("fps");
    time = new TextField ("time");
    noc = new TextField ("number of clusters");
    isCollapsed = true;
    slideX = 0;
    initialX = 0;
    currentTime = 0;
  ...
  private void setAllPos (float x) {
    ...
    fps.setPosition (x, 460);
    time.setPosition (x, 500);
    noc.setPosition (x, 540);
  }
  ...
  public void update () {
    ...
    if (slideX > 0) {
      ...
      pict.update ();
      regene.update ();
      fps.setContent ("" + frameRate);
      fps.update ();
      time.setContent ("" + currentTime);
      time.update ();
      noc.update ();
    ...
    popMatrix();
    textAlign (LEFT, CENTER);
    text ("Diffusion Limited Aggregation Simulator v 6", 10 , height - 15);
  }
